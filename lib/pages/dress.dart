...
class _DressState extends State<Dress> {
  ...
  Future<void> _fetchData() async {
    try  
{
      dressModel = await _apiProvider.getdresses();
           if (mounted)setState(() {
        isLoading = false;
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              (dressModel?.products != null) ? ListView.separated(
                          shrinkWrap: true,
                          physics: const NeverScrollableScrollPhysics(),
                          itemBuilder: (context, index) {
                            return buildproductItem(product:dressModel!.products![index]);
                          },
                          separatorBuilder: (context, index) => const SizedBox(height: 10),
                          itemCount: dressModel!.products!.length,
                        )
